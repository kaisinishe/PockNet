    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage == "test" and self.hparams.eval_dataset == "bu48":
            if not self.data_test:
                print(
                    f"Loading bu48 evaluation dataset from {self.hparams.data_dir}/bu48/vectorsEval.csv"
                )
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            ...
            X_train_val, X_test, y_train_val, y_test = train_test_split(
                X, y, test_size=self.hparams.train_val_test_split[2], stratify=y, random_state=42
            )
